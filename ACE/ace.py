...
class ConceptDiscovery(object):
  ...
  def _return_superpixels(self, img, method='slic',
    ...
    if param_dict is None:
      param_dict = {}
    if method == 'slic':
      n_segmentss = param_dict.pop('n_segments', [70, 15, 15])
      n_params = len(n_segmentss)
      compactnesses = param_dict.pop('compactness', [50] * n_params)
    ...
    for i in range(n_params):
      param_masks = []
      if method == 'slic':
        print(".........................",n_segmentss,compactnesses)
  ...
  def load_cav_direction(self, c, r, bn, directory=None):
    ...
    if directory is None:
      directory = self.cav_dir
    cav_hparams = cav.CAV.default_hparams()
    # params = tf.contrib.training.HParams(model_type='linear', alpha=.01)
    cav_key = cav.CAV.cav_key([c, r], bn, cav_hparams['model_type'],
                    cav_hparams['alpha'])
    ...
    for bn in self.bottlenecks:
      ...
      for i in range(len(acts)):
        bn_grads[i] = self.model.get_gradient(
            acts[i:i+1], [class_id], bn,1).reshape(-1)
