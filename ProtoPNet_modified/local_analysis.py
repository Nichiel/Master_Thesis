...
save_analysis_path = '/content/drive/MyDrive/MG/work_space/ProtoPNet/saved_models/vgg19/003/10nopush0.9626.pth'# os.path.join(test_image_dir, model_base_architecture, experiment_run, load_model_name)
...
prototype_info =np.load('/content/drive/MyDrive/MG/work_space/ProtoPNet/saved_models/vgg19/003/img/epoch-10/bb10.npy') #os.path.join(load_img_dir, 'epoch-'+epoch_number_str, 'bb'+epoch_number_str+'.npy'))
...
def save_prototype(fname, epoch, index):
    p_img = plt.imread(os.path.join('/content/drive/MyDrive/MG/work_space/ProtoPNet/saved_models/vgg19/003/img/epoch-10', 'prototype-img'+str(index)+'.png'))
    #plt.axis('off')
    plt.imsave(fname, p_img)
    
def save_prototype_self_activation(fname, epoch, index):
    p_img = plt.imread(os.path.join('/content/drive/MyDrive/MG/work_space/ProtoPNet/saved_models/vgg19/003/img/epoch-10',
...
def save_prototype_original_img_with_bbox(fname, epoch, index,
    ...
    p_img_bgr = cv2.imread(os.path.join('/content/drive/MyDrive/MG/work_space/ProtoPNet/saved_models/vgg19/003/img/epoch-10', 'prototype-img-original'+str(index)+'.png'))
...
ma_folder = "most_activated_prototypes" + str(test_image_name) + str( args.imgclass[0])
makedir(os.path.join(save_analysis_path, ma_folder))

log('Most activated 10 prototypes of this image:')
array_act, sorted_indices_act = torch.sort(prototype_activations[idx])
for i in range(1,11):
    log('top {0} activated prototype for this image:'.format(i))
    save_prototype(os.path.join(save_analysis_path, ma_folder,
                                'top-%d_activated_prototype.png' % i),
                   start_epoch_number, sorted_indices_act[-i].item())
    save_prototype_original_img_with_bbox(fname=os.path.join(save_analysis_path, ma_folder,
                                          ...
                                          color=(0, 255, 255))
    save_prototype_self_activation(os.path.join(save_analysis_path, ma_folder,
    ...
    #plt.axis('off')
    plt.imsave(os.path.join(save_analysis_path, ma_folder,
                            'most_highly_activated_patch_by_top-%d_prototype.png' % i),
               high_act_patch)
    log('most highly activated patch by this prototype shown in the original image:')
    imsave_with_bbox(fname=os.path.join(save_analysis_path, ma_folder,
...
for i,c in enumerate(topk_classes.detach().cpu().numpy()):
    ...
    for j in reversed(sorted_indices_cls_act.detach().cpu().numpy()):
        ...
        print("prototype text",prototype_index)
        if prototype_index < 100:
